...
const Step4 = props => {
  ...
  return (
    <React.Fragment>
      <div className="block">
        <h2>Опыт работы</h2>
        <FormItem {...formItemLayout} label="Без опыта">
          <Switch
            checked={props.isExperienced}
            onChange={props.handleExperienceToggle}
          />
        </FormItem>
        ...
        {props.data.length > 0 ? (
          <Timeline className="m-5 preview-experience">
            {props.data.map(item => (
              <Timeline.Item
                key={Math.random()}
                className="preview-experience_item"
              >
                <Button
                ...
                <div className="preview-experience-date">
                  С {moment(item.dateStart).format("MMMM YYYY")} по{" "}
                  {moment(item.dateEnd).format("MMMM YYYY")}
                </div>
          ...
          <span style={{ textAlign: "center" }}>
            Добавьте хотя бы один раз информацию об опыте работы, чтобы увидеть
            превью
          </span>
        )}
      </div>
