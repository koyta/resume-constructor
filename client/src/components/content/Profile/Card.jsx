    ...
    return (
      <div className={cx(this.state.cardClass)}>
        <header className="profile-card-header">
          ...
          <Link to={`/resume/view/${resume._id}`} className="profile-card-link"><Icon type="global" /></Link>
        </header>
        <div className="profile-card-content">
          <div className="profile-card-experience">Опыт работы: {experienceString}</div>
          {(github || medium) &&
          <div className="profile-card-socials">
            <span>Социальные профили:</span>
            {github && <a href={`https://github.com/${github}`}><Icon type="github" /></a>}
            {medium && <a href={`https://medium.com/@${medium}`}><Icon type="medium " /></a>}
          </div>}
        </div>
      </div>
    );
